...
For basemap display, the metric array is reprojected onto a regular
**Web Mercator (EPSG:3857)** grid via `_regrid_to_mercator()` in `datacube_io.py`
(nearest-neighbour lookup, pyproj EPSG:3857 → EPSG:32734).  Working natively in
Mercator metres ensures each PNG pixel maps to an equal-area Mercator cell, which
matches exactly how `ipyleaflet.ImageOverlay` stretches the image onto the tile
basemap.  WGS84 lat/lon coordinates are derived algebraically from the Mercator
grid and used only for the `ImageOverlay` bounds.

Pixel click-to-select is unaffected — that path converts ipyleaflet lat/lon
directly back to UTM and never touches the overlay grid.
...
The metric array is reprojected from the UTM source grid onto a regular
**Web Mercator (EPSG:3857)** target grid via `_regrid_to_mercator()`:

1. A `scipy.interpolate.RegularGridInterpolator` is built on the UTM source
   grid (sorted axes, nearest-neighbour method).
2. A regular target grid is defined in Mercator metres — equal-area cells in
   both axes — sized to the same pixel count as the (independently coarsened)
   source array.
3. Each Mercator cell centre is transformed to UTM via pyproj
   (EPSG:3857 → EPSG:32734) and looked up in the interpolator.
4. WGS84 lat/lon for the `ImageOverlay` bounds is derived algebraically from
   the Mercator grid (`lon = merc_x / R · (180/π)`,
   `lat = 2 arctan(exp(merc_y / R)) − 90°`).

The resulting PNG is served via `ipyleaflet.ImageOverlay`.  Because the PNG rows
are evenly spaced in Mercator-y and columns in Mercator-x, Leaflet's linear
image stretch is geometrically exact — each pixel footprint maps precisely onto
the tile basemap.

A satellite tile layer (default: ESRI World Imagery) is displayed underneath.
The tile service, overlay opacity, and **Data range** clipping (full / Mean ± N SD)
...
nearest-neighbour matched to an array index `(yi, xi)`.  This path is entirely
independent of the overlay grid.
...
| pyproj | current | CRS reprojection (UTM ↔ Web Mercator; WGS84 bounds derivation) |
...
- **Coordinate reprojection accuracy** — UTM→Mercator nearest-neighbour reprojection
  introduces at most 0.5 source pixel of positional error (~15 m for 30 m data).
  Leaflet's `ImageOverlay` stretch is geometrically exact because the PNG rows/columns
  are natively spaced in Mercator metres.
- **Basemap downsampling** — The on-the-fly compute path downsamples each axis
  independently to ≤ `BASEMAP_MAX_DIM` pixels (default 500) before reprojection.
  The precomputed `pixel_metrics.nc` path uses `BASEMAP_MAX_DIM_PRECOMPUTED` (default
  2 000), displaying most regions at or near native 30 m resolution.  Sub-pixel
  spatial detail below the display resolution is not visible; the full-resolution
  pixel index is always used for time series extraction.
- **Data range filter and phenology metrics** — When a phenology basemap metric
  (e.g., Season Length, Peak DOY) is active, the Data range colorscale clipping is
  display-only and does **not** filter VI observations.  Filtering is only applied
  when a VI-scaled quick metric (Peak VI, Mean VI, Std Dev VI, Data Coverage) is
  displayed, since only then are the colorscale limits in the same units as the VI
  observations.
